...
def headReport(config, report_temp):
    ...
    head_dir = os.path.join(report_temp, 'html/head.html')
...
def qcReport(config, report_temp):
    ...
    fastqc_table = transformTable2html(fastqcMD_dir, 
                                       'Quality statistics of raw sequence data')
    fastpMD_dir = 'Report/Table/fastp_summary_report.xls'
    fastp_table = transformTable2html(fastpMD_dir, 
                                       'Quality statistics after quality control')
    head_dir = os.path.join(report_temp, 'html/QC.html')
    with open(head_dir, 'r') as f:
        head_text = f.read()
...
def countReport(config, report_temp):
    ...
    countMat_table = transformTable2html(countMat_dir, 
                                       'Example of gene expression matrix results')
    head_dir = os.path.join(report_temp, 'html/Count.html')
    with open(head_dir, 'r') as f:
        head_text = f.read()
...
def get_figure_text(img_dir, img_title):
    out_text = '<div align="center"> FigureNum {}</div>'.format(img_title)
    out_text = out_text + '<div align="center"><img src={} height="500" width="600"/></div>'.format(img_dir)
    return out_text

def gVirReport(config, report_temp):
    pca_dir = 'Report/Figure/PCA_plot.png'
    pca_fig = get_figure_text(pca_dir, 'PCA result diagram')
    nmds_dir = 'Report/Figure/NMDS_plot.png'
    nmds_fig = get_figure_text(nmds_dir, 'NMDS result diagram')    
    head_dir = os.path.join(report_temp, 'html/GVir.html')
    with open(head_dir, 'r') as f:
        head_text = f.read()
...
def deaReport(config, report_temp):
    ...
    for item in compare_text:
        ...
        degMat_table = transformTable2html(degMat_dir, deg_title)
        degRes_text = degRes_text + degMat_table
        heatmap_dir = 'Report/Figure/heatmap_{}.png'.format(item)
        ...
        volcanoPlot_dir = 'Report/Figure/volcanoPlot_{}.png'.format(item)
    ...
    head_dir = os.path.join(report_temp, 'html/DEA.html')
    with open(head_dir, 'r') as f:
        head_text = f.read()
...
def enrichmentReport(config, report_temp):
    ...
    goRes_text = ''
    for item in compare_text:
        kegg_dir = 'Report/Figure/DEG_KEGG_enrichment_{}.png'.format(item)
        ...
        go_dir = 'Report/Figure/DEG_GeneOntology_BP_enrichment_{}.png'.format(item)
    ...
    head_dir = os.path.join(report_temp, 'html/enrichment.html')
    with open(head_dir, 'r') as f:
        head_text = f.read()
...
def generateFinalReport(config, report_temp):
    ...
    return report_text
html_text = generateFinalReport(config, report_temp)
